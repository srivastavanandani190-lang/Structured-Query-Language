...
--Foreign key constraint to ensure that each address is associated with a valid user.
--foreign key constraint is set to cascade on delete, meaning that if a user is deleted, all their associated addresses will also be deleted automatically.
--there are other options for handling foreign key constraints, such as restricting deletion or setting the foreign key to null.
);
